...
public class SmartGridSimulation extends CoupledModelImpl implements StateVariableBased{ 
	private static final long serialVersionUID = 1L;
	protected SimulationOptionsImpl options = new SimulationOptionsImpl();
	
	public SmartGridSimulation(){
		this("SmartGridSimulation");
	}
	public SmartGridSimulation(String nm) {
		super(nm);
		make();
	}
	public void make(){
		...
		addChildModel(PainelSolar);
		Medidor Medidor = new Medidor();
		addChildModel(Medidor);
		Residencia Residencia = new Residencia();
		addChildModel(Residencia);
		CentralEnergia CentralEnergia = new CentralEnergia();
		addChildModel(CentralEnergia);
		addCoupling(Medidor.outConsumo,CentralEnergia.inConsumo);
		addCoupling(PainelSolar.outProducao,Residencia.inProducao);
		addCoupling(Residencia.outProducao,Medidor.inProducao);
		addCoupling(Residencia.outConsumo,Medidor.inConsumo);
	...
	public static void main(String[] args){
		...
		SmartGridSimulation model = new SmartGridSimulation();
		model.options = options;
		if(options.isDisableViewer()){ // Command Line output only
			Simulation sim = new com.ms4systems.devs.core.simulation.impl.SimulationImpl("SmartGridSimulation Simulation",model,options);
