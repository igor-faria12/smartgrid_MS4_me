...
DNL
platform:/resource/smartgrid_v2/src/Models/dnl/Medidor.dnl
-1886664842
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class Medidor extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public Medidor(String name, Simulator simulator) {
    ...
    public MessageBag getOutput() {
        ...
        if (phaseIs("s2")) {
            // Output event code
            //ID:OUT:s2
            medicaoConsumo.setValue(medicaoConsumo.getValue() -
                medicaoProducao.getValue());
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("Medidor Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = Medidor.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                Medidor.class.getResource(".").toString());
        }
