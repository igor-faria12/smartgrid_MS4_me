...
public class Medidor1 extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public Medidor1(String name) {
        this(name, null);
    }

    public Medidor1(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        Medidor1 model = new Medidor1();
        model.options = options;

        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("Medidor1 Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = Medidor1.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                Medidor1.class.getResource(".").toString());
        }
